import { useEffect, useState, type ChangeEvent, type FormEvent, type InputHTMLAttributes, type TextareaHTMLAttributes } from "react";
import { motion } from "framer-motion";
import { adminApiClient, apiClient } from "@/api";
import { nearestColorName } from "@/lib/colorNames";
import type { ColorSwatch } from "@/models/color";
import type { AboutContent, AboutDetail, ContactInfo } from "@/models/content";
...
const blankContact: ContactInfo = {
  eyebrow: "",
  heading: "",
  body: "",
  button: "",
...
function validateProject(project: ProjectFormState): string {
...
function validateProduct(product: ProductFormState): string {
  const required: [string, string][] = [
...
export default function AdminDashboard() {
  ...
  const [aboutForm, setAboutForm] = useState<AboutContent>(blankAbout);
  const [contactForm, setContactForm] = useState<ContactInfo>(blankContact);
  ...
  async function loadProjects() {
    setError("");
    setProjects(await adminApiClient.listProjects());
  }

  async function loadAbout() {
    setError("");
    setAboutForm(await adminApiClient.getAbout());
  }

  async function loadContact() {
    setError("");
    setContactForm(await adminApiClient.getContactInfo());
  }

  async function loadProducts() {
    setError("");
    setProducts(await adminApiClient.listProducts());
  }
  ...
  function updateAboutList(list: "details" | "stats", index: number, field: keyof AboutDetail, value: string) {
  ...
  async function uploadFiles(files: File[]): Promise<{ urls: string[]; failures: string[] }> {
    const urls: string[] = [];
    const failures: string[] = [];
    for (const file of files) {
      try {
        const { url } = await adminApiClient.uploadFile(file);
        urls.push(url);
      } catch (caught) {
  ...
  function removeProjectImage(image: string) {
    setForm((current) => ({
      ...current,
  ...
  function updateProductColor(index: number, field: "name" | "hex", value: string) {
    setProductForm((current) => ({
      ...current,
      colors: current.colors.map((color, colorIndex) => {
        if (colorIndex !== index) return color;
    ...
    <main className="min-h-screen bg-cream px-[clamp(1rem,4vw,4rem)] py-8 text-ink">
      ...
      {isContactPage ? (
        ...
        <form className="mx-auto grid max-w-4xl gap-5 rounded-lg border border-ink/15 bg-white p-5" onSubmit={saveAbout}>
          ...
          <TextAreaField label="Body paragraphs" required value={(Array.isArray(aboutForm.body) ? aboutForm.body : [aboutForm.body]).join("\n\n")} onChange={(event) => setAboutForm({ ...aboutForm, body: event.target.value.split(/\n\n+/).map((item) => item.trim()).filter(Boolean) })} />
          ...
          <div className="grid gap-3">
            {products.map((product) => (
              <button className={`grid cursor-pointer grid-cols-[3.5rem_1fr_auto] gap-3 rounded-md border p-3 text-left transition hover:-translate-y-0.5 hover:border-star hover:shadow-[0_10px_24px_rgba(61,48,70,.12)] ${editingProductId === product.id ? "border-star bg-[#f9effb]" : "border-ink/10 bg-cream"}`} key={product.id} onClick={() => editProduct(product)} type="button">
                {product.image ? (
                  <img className="size-14 rounded object-cover" src={apiClient.assetUrl(product.image)} alt="" />
                ) : (
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {(productForm.images || []).map((image) => (
                <div className="overflow-hidden rounded-md border border-ink/15 bg-cream transition hover:-translate-y-0.5 hover:border-star hover:shadow-[0_10px_24px_rgba(61,48,70,.12)]" key={image}>
                  <img className="h-36 w-full object-cover" src={apiClient.assetUrl(image)} alt="" />
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(18rem,25rem)_1fr]">
          ...
          <div className="grid gap-3">
            {projects.map((project) => (
              <button className={`grid cursor-pointer grid-cols-[4.5rem_1fr] gap-3 rounded-md border p-2 text-left transition hover:-translate-y-0.5 hover:border-star hover:shadow-[0_10px_24px_rgba(61,48,70,.12)] ${editingId === project.id ? "border-star bg-[#f9effb]" : "border-ink/10 bg-cream"}`} key={project.id} onClick={() => editProject(project)} type="button">
                <img className="h-20 w-full rounded object-cover" src={apiClient.assetUrl(project.image)} alt="" />
                <span className="min-w-0 self-center">
        ...
        <form className="grid gap-5 rounded-lg border border-ink/15 bg-white p-5" onSubmit={saveProject}>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {(["yarn", "fiber", "technique", "needles", "size"] as const).map((field) => (
