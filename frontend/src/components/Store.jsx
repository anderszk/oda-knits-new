...
function ProductCard({ product, index, columns, onSelect, stagger }) {
  ...
  const { ref: cardRef, style: revealStyle } = useScrollReveal({
    from: { x: dirX, y: 46 },
    rotate: dirRotate,
    scale: 0.94,
  });
  ...
  const liftedY = useTransform(
    ...
    ([scrollY, lift]) => scrollY + baseOffset + lift,
  );
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none relative aspect-square w-full overflow-hidden border-b border-ink/10" style={product.image ? undefined : swatchStyle(product.colors)}>
        {product.image ? (
          <img className="absolute inset-0 size-full object-cover transition-transform duration-500 group-hover:scale-[1.045]" src={assetUrl(product.image)} alt="" />
        ) : (
