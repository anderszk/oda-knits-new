...
export default function InstagramCarousel() {
  ...
  return (
    <section
      className="border-y border-ink/10 bg-[#f3efe7] px-[clamp(1rem,4vw,4rem)] py-18 max-[620px]:px-4 max-[620px]:py-12"
      id="instagram"
    >
...
function InstagramPostCard({ index, post }) {
  const path = postPaths[index % postPaths.length];
  const reveal = useScrollReveal({ from: { x: path.x, y: path.y }, rotate: path.rotate, scale: 0.97 });
